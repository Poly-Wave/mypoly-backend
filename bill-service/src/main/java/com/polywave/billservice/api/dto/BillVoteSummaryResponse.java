...
public record BillVoteSummaryResponse(
                ...
                @Schema(description = "현재 사용자의 투표값. hasVoted=false인 경우 아직 투표하지 않은 상태이므로 null입니다. 가능한 값: AGREE, DISAGREE", example = "AGREE", nullable = true) String myVoteResult,

                @Schema(description = "찬성 수", example = "120") long agreeCount,

                @Schema(description = "반대 수", example = "80") long disagreeCount,

                @Schema(description = "총 투표 수", example = "200") long totalVoteCount,

                @Schema(description = "찬성 비율 (0~1)", example = "0.60") double agreeRatio,
                @Schema(description = "반대 비율 (0~1)", example = "0.40") double disagreeRatio) {
