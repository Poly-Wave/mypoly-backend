...
public interface CategoryApi {
            ...
            @io.swagger.v3.oas.annotations.responses.ApiResponse(
                    ...
                    content = @Content(
                            ...
                            examples = @ExampleObject(
                                    name = "조회 성공",
                                    value = CategoryApiExamples.EXAMPLE_GET_CATEGORIES_OK
                            )
                    )
            ),
            @io.swagger.v3.oas.annotations.responses.ApiResponse(
    ...
    @Operation(summary = "내 관심 카테고리 저장(갱신)", description = """
            ...
            동작 규칙
            - 요청으로 받은 `categoryCodes`를 기준으로 **사용자의 관심 카테고리를 갱신**합니다.
              - 기존 관심사 중 요청에 없는 항목은 삭제
              - 요청에 새로 추가된 항목은 추가
            - **중복 코드는 자동으로 제거**됩니다.
            - **존재하지 않거나 비활성화된 카테고리 코드는 무시**됩니다. (에러로 실패시키지 않음)
    ...
    @io.swagger.v3.oas.annotations.responses.ApiResponses({
            ...
            @io.swagger.v3.oas.annotations.responses.ApiResponse(
                    ...
                    content = @Content(
                            ...
                            examples = {
                                    @ExampleObject(
                                            name = "요청 값 검증 실패",
                                            value = CategoryApiExamples.EXAMPLE_BAD_REQUEST_VALIDATION
                                    ),
                                    @ExampleObject(
                                            name = "온보딩 상태 오류",
                                            value = CategoryApiExamples.EXAMPLE_INVALID_ONBOARDING_STATUS
                                    )
                            }
                    )
            ),
            ...
            @io.swagger.v3.oas.annotations.responses.ApiResponse(
                    ...
                    content = @Content(
                            ...
                            examples = {
                                    @ExampleObject(
                                            name = "사용자 서비스 API 호출 실패",
                                            value = CategoryApiExamples.EXAMPLE_USER_SERVICE_API_FAILED
                                    ),
                                    @ExampleObject(
                                            name = "서버 오류",
                                            value = CommonApiExamples.EXAMPLE_INTERNAL_SERVER_ERROR
    ...
    ResponseEntity<Void> updateInterests(
            ...
            @io.swagger.v3.oas.annotations.parameters.RequestBody(
                    ...
                    content = @Content(
                            ...
                            examples = @ExampleObject(
                                    name = "요청 예시",
                                    value = CategoryApiExamples.EXAMPLE_UPDATE_INTERESTS_REQUEST
                            )
                    )
            )
