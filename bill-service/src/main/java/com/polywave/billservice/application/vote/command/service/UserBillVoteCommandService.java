...
public class UserBillVoteCommandService {
    ...
    @Transactional
    public void voteOnBill(Long userId, Long billId, String voteResult, String voterAgeBand, Long existingVoteId) {
        Instant now = Instant.now();
        if (existingVoteId == null) {
            Bill bill = billCommandRepository.findById(billId)
                    .orElseThrow(BillNotFoundException::new);
            UserBillVote newVote = UserBillVote.create(userId, bill, voteResult, voterAgeBand, now);
        ...
        existingVote.changeVote(voteResult, voterAgeBand, now);
    }
}
