...
public interface DevAuthApi {
        ...
        @Operation(summary = "[DEV/LOCAL] Swagger 테스트용 JWT 발급", description = """
        ...
        ResponseEntity<SocialLoginResponse> devLogin(
                        @Parameter(in = ParameterIn.HEADER, description = "DEV 키 (local 기본값: local-dev-key, 배포 시 DEV_AUTH_KEY로 주입)", example = "local-dev-key", required = true) @RequestHeader("X-DEV-KEY") String devKey);
}
